...
**All 78 items are closed, and no known defect is open.** The only two not *implemented* are
Round 1's **A5** and **C5**, left by decision with measured arguments in the code against them.
**S1** is likewise closed as a decision *not to change anything*: the proposed fix was measured
on production data and made the problem worse.
...
| Round 3 | O, P, Q, R | 13 | ✅ all closed — **R3**'s premise was wrong, pinned by test; **O3**, **Q3** and **P4** were found *during* the round and fixed in it |
| Round 3 | S, T | 4 | ✅ all closed 2026-09-06 — **S1** decided *not* to change (measured), **S2**/**S3** shipped, **T1** covered |
| Follow-through | K1 (`file_path` half) | 1 | ✅ closed 2026-09-06 — M1's stated unblock had never been delivered; surfaced by T1 |

**Four defects were surfaced by finishing this round, and none of them could have been caught by
a test**: **O3** (`manual-link` accepted ids naming no record), **Q3** (a stored field that could
never be read back), **P4** (`/api/match/status` reporting "not found" for any batch that was not
COMPLETED), and **J5** (a 64 MB `/dev/shm` that failed with a message naming disk). Each came from
running the system, and three of the four were **Postgres-only divergences invisible to the
...
## EPIC P — Execution visibility (H) — ✅ complete (P4 added and closed 2026-09-06)
...
| ~~P4~~ ✅ | `GET /api/match/status` 404s for any batch that is not COMPLETED | **Found 2026-09-06 while covering T1 — a defect in P1's own shipping.** `PostgresStore.GetProgress` scanned `match_jobs.completed_at` into a non-pointer `time.Time`. That column is NULL until a run finishes, a NULL cannot scan into `time.Time`, and the function flattens its error into a bool — so the scan failed and surfaced as **"batch not found"** for every batch that was uploaded-but-never-matched *and* every batch **currently RUNNING**. A caller could not distinguish a live job from a missing one. P1's UI fallback treats 404 as "reset to idle", which is coincidentally right for a never-matched batch and **wrong for a running one**. Fixed by scanning into a `*time.Time`. **Postgres-only by nature** — the in-memory store holds a struct with no NULL to mis-scan, which is exactly why the entire suite stayed green while the deployed path was broken: the same store-divergence class as K4, M5, I2 and O3, now five occurrences. Mutation-checked against a real database; the test skips without `TEST_DATABASE_URL`, so a green default suite does not prove it |
...
## EPIC T — Coverage gaps from this sweep (M) — ✅ complete

| ID | Story | AC |
| :-- | :-- | :-- |
| ~~T1~~ ✅ | Cover the five controls this sweep could not reach | **File pickers** ("Choose File", Upload & Run Matching) need a real file through the browser dialog. **Load Data & Start Batch** was clicked and surfaced an error box that could not be captured before reload; it created no batch and logged nothing server-side. **The 4,000-record stress test** was skipped to avoid loading the box mid-sweep. **Connector success paths** — Test Connection and Introspect only ever reached their failure branch, since `CONNECTOR_FILE_ROOT` is unset and no live database is configured. **Export CSV** was verified at the endpoint (191,425 rows) but not clicked through to a browser download. None are known-broken; all are untested. **All five covered on 2026-09-06 against the composed stack, and covering them found two real defects — neither of which any test could have caught.** (1) **File pickers**: real files driven through the actual browser dialog, then Upload & Run Matching — batch created, match ran, dashboard rendered `COMPLETED` 4/4. (2) **Load Data & Start Batch** failed for a reason nobody had established: `HandleConnectorIngest` refused CSV/Excel outright, and its own comment said it "should accept file paths only once [M1] lands". **M1 landed in Round 2 and the follow-through was never done**, so the button was dead by construction — the file_path half of K1 that M1's entry claims to unblock. Now opened, routed through the *same* `resolveConnectorFilePath` helper (never re-implemented), checking **both** sides because a caller controls both, and writing the resolved path back so the connector opens exactly what was validated rather than re-deriving it. Verified live: ingest succeeds inside the root, and `/etc/passwd` is refused with the side named. (3) **4,000-record stress test**: 2,420 sources → 6,685 pairs, 1,118 auto / 5,539 review / 28 unmatched in **429ms**. (4) **Connector success paths**: Test Connection and Introspect both reached success for the first time, and M1 containment was re-confirmed against `/etc/passwd` and `../../etc/passwd`. (5) **Export CSV** through nginx: `200`, `text/csv`, `Content-Disposition: attachment`, correct header row; the audit export too. **Enabling any of this needed `CONNECTOR_FILE_ROOT`, which compose never set** — so the whole file-connector feature shipped unusable. `docker-compose.yml` now sets it to a **read-only** mount of `connector-data/`, keeping the blast radius to that directory |
